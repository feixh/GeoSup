...
def disp_net_visma(opt, dispnet_inputs, is_training):
    # is_training = opt.mode == 'train_rigid'
    def get_disp_visma(x):
        # inverse depth ranges [1/(SCALE+OFFSET), 1/OFFSET]
        SCALE = 10.0
        OFFSET = 0.2
        return SCALE*slim.conv2d(x, 1, 3, 1, activation_fn=tf.nn.sigmoid, normalizer_fn=None) + OFFSET

    if opt.dispnet_encoder == 'vgg':
        return build_vgg(dispnet_inputs, get_disp_visma, is_training, 'depth_net')
    else:
        return build_resnet50(dispnet_inputs, get_disp_visma, is_training, 'depth_net')
